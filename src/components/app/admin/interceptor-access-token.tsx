    ...
    const interceptorId = API.interceptors.response.use(
      ...
      (error: AxiosError) => {
        if (error.isAxiosError) {
          ...
          if ((status === 401 || status === 403) && !toastDisplayed.current) {
            // Exibe o toast e redireciona
            toast.error('Acesso Proibido!', {
              description:
                'Você precisa ser um administrador e deve estar logado.',
            })

            navigate('/restrict/admin/login?token=false', { replace: true })
